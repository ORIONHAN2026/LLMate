...
LLMWork 为企业提供可视化的大模型接口管理与代理服务，帮助您按部门、按岗位精细化分配 AI 资源。以下是完整的入门指南。


#### 1. 安装包下载

访问 [下载页面](download.html) 获取最新版本的安装包。
...
LLMWork 支持接入主流大模型供应商，统一管理企业的所有 AI 模型资源。

##### 添加模型供应商

1. 打开 LLMWork，点击左侧菜单中的 **模型管理**
2. 点击 **添加模型** 按钮
3. 填写以下配置项：
...
会话是 LLMWork 的核心管理单元，每个会话对应一个独立的工作场景和资源配置。
...
LLMWork 内置本地 HTTP/HTTPS 代理服务，将配置好的大模型接口透传为 OpenAI 兼容 API。
...
LLMWork 提供完善的用量统计与配额管理功能，帮助企业精确控制 AI 资源消耗。
...
> **下一步**：阅读 [功能说明](feature-guide.html) 了解 LLMWork 的完整功能列表。
