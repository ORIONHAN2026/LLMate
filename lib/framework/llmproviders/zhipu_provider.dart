...
class ZhipuProvider extends BaseLlmProvider {
  ...
  Stream<Map<String, String?>> sendMessageStream({
    ...
    try {
      ...
      if (response.statusCode == 200 && response.data != null) {
        yield* _processZhiPuStreamResponse(response.data!.stream);
      } else {
  ...
  Stream<Map<String, String?>> sendMessageStreamWithMessages(
    ...
    try {
      ...
      if (kDebugMode) {
        print('智谱AI (withMessages) 发送请求到: ${model!.apiUrl}');
      }
  ...
  Future<String?> sendMessage({
    ...
    try {
      ...
      if (kDebugMode) {
        print('智谱AI 发送非流式请求到: ${model!.apiUrl}');
        print('请求数据: ${jsonEncode(requestData)}');
      }
      ...
      if (kDebugMode) {
        print('智谱AI 非流式响应错误: $e');
      }
      throw Exception('智谱AI API 错误: ${handleApiError(e)}');
    }
  }
  ...
  /// 处理 ZhiPu 流式响应
  Stream<Map<String, String?>> _processZhiPuStreamResponse(Stream<List<int>> stream) async* {
    String buffer = '';
      ...
      for (final line in lines) {
        if (line.trim().startsWith('data: ')) {
          ...
          try {
            ...
            final content = _extractZhiPuContent(data);
            ...
            // 忽略解析错误，继续处理下一行
            if (kDebugMode) {
              print('ZhiPu JSON解析错误: $e, 行内容: $line');
            }
          }
        }
      }
  ...
  /// 从 ZhiPu 流式数据中提取内容
  String? _extractZhiPuContent(Map<String, dynamic> data) {
    try {
      final choices = data['choices'] as List?;
      if (choices != null && choices.isNotEmpty) {
        final choice = choices[0] as Map<String, dynamic>;
